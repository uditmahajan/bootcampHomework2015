...
void drawDot(float x, float y, boolean isRed) {
  float d = map(sqrt(square(x-mouseX) + square(y-mouseY)), 70, 0, 10, 50); //mouse over range, circle size
  if (d<10) {d = 10;
  }
  // if outside the grid and red or less than 20, then make big
  if(isRed && (d < 20)) { //big dot
    ellipse(x, y, 20, 20);
  }
